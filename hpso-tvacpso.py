# HPSO AND TVAC PSO 
# no inertial term, if v=0, reinitialize velocity to vmax at beginning, 0.1*vmax at end
...
class Particle:
    def __init__(self, initial):
        ...
        self.fit = -1
        self.vmax = 1
        self.c1i, self.c2i = 2.5, 0.5
        self.c1f, self.c2f = 0.5, 2.5
    ...
    def update_velocity(self, global_best, maxiter, iter):
        w = 0.5        # inertial weight
        c1 = self.c1i + (self.c1f - self.c1i)/maxiter * iter      # cognitive factor (local best)     
        c2 = self.c2i + (self.c2f - self.c2i)/maxiter * iter      # social factor (global best)
        ...
        vel = list(cognitive + social)
        if abs(vel[0]) <= 0.00001:
            vel[0] = math.copysign(self.vmax, vel[0])
        if abs(vel[1]) <= 0.00001:
            vel[1] = math.copysign(self.vmax, vel[1])
        if abs(vel[0]) > 1:
            vel[0] == math.copysign(1,vel[0])
        if abs(vel[1]) > 1:
            vel[1] == math.copysign(1,vel[1])
        self.vel = vel
        self.vmax = self.vmax - (self.vmax - 0.1*self.vmax)/maxiter * iter
...
class PSO():
    ...
    def optimize(self):
        ...
        for i in range(self.maxiter):
            ...
            for j in range(self.num_particles):
                self.swarm[j].update_velocity(self.global_best_pos, self.maxiter, i)
